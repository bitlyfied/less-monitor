...
var Monitor = new Class({
	...
	init: function( options ) {
		...
		this.options.include = Array.from(this.options.include);

		this.options.include.push(this.options.directory);

		this.options.include = this.options.include.map(function(includePath){
			return path.resolve(path.relative( process.cwd(), includePath));
		});
	...
	/**
	 * Search the file in the include paths. Returns absolute path
	 *
	 * @param  {string} file  Full path of the file
	 * @return {string}       Absolute path of the file
	 */
	findInPaths: function( file ) {
		var filePath = this.getRelativePath( file );
		...
		for(var i = 0; i < this.options.include.length; i++){
			filePath = path.join( this.options.include[i], file );
			if(fs.existsSync(filePath)){
				return path.relative( this.options.directory, filePath);
			}
		}

		this.emit( 'error', { message: 'Could not find file "' + file + '"' });
	},
	...
	outputMap: function( files ) {
		...
		Array.from( files ).each( function ( file ) {
			if(this.options.output){
				map[ file ] = path.join( this.options.output +  path.dirname( file ), path.basename( file, inExt ) ) + outExt;
				console.log(map[ file ]);
			}else{
				map[ file ] = path.join( path.dirname( file ), path.basename( file, inExt ) ) + outExt;
			}
		}, this);
	...
	isExpired: function( file, callback ) {
		...
		fs.stat( filePath, function( err, stat ) {
			if ( err ) {
				this.emit( 'error', { message: 'IsExpired: Could not open file "' + file + '"' });
				return;
			} else {
		...
		this.isExpired( file, function( expired ) {
			...
			if ( expired ) {
				...
				fs.readFile( filePath, 'utf8', function( err, data ) {
					...
					if ( err ) {
						this.emit( 'error', { message: 'LookupFile: Could not open file "' + file + '"' });
					} else {
						...
						imports = imports.map( function( f ) {
							var fileName = path.extname( f ) ? f  : f + '.less';
							return self.findInPaths(fileName);
						});
	...
	parse: function( files, options ) {
		...
		Array.forEach( files, function( file ) {
			...
			fs.readFile( filePath, 'utf-8', function( err, data ) {
				...
				if ( err ) {
					this.emit( 'error', { message: 'Parse: Could not open file "' + file + '"' });
				} else {
