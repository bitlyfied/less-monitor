...
var Monitor = new Class({
	...
	isExpired: function( file, callback ) {
		...
		fs.stat( filePath, function( err, stat ) {
			if ( err ) {
				this.emit( 'error', { message: 'C Could not open file "' + file + '"' });
				return;
			} else {
		...
		this.isExpired( file, function( expired ) {
			...
			if ( expired ) {
				...
				fs.readFile( filePath, 'utf8', function( err, data ) {
					...
					if ( err ) {
						this.emit( 'error', { message: 'B Could not open file "' + file + '"' });
					} else {
	...
	parse: function( files, options ) {
		...
		Array.forEach( files, function( file ) {
			...
			fs.readFile( filePath, 'utf-8', function( err, data ) {
				...
				if ( err ) {
					this.emit( 'error', { message: 'A Could not open file "' + file + '"' });
				} else {
